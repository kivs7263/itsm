...
import type { CI, Customer, CustomersResponse } from '@/lib/types';
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
type EditCIValues = z.infer<typeof editCISchema>;
...
// -----------------------------------------------------------------------
// EditCIModal — PUT /{tenant}/cmdb/cis/{id}
// -----------------------------------------------------------------------
export function EditCIModal({ open, onClose, tenantSlug, ci }: EditCIModalProps) {
  const queryClient = useQueryClient();

  const descriptionDefault = typeof ci.attributes?.description === 'string'
    ? ci.attributes.description
    : '';
  ...
  } = useForm<EditCIValues>({
    resolver: zodResolver(editCISchema),
    defaultValues: {
      name:        ci.name,
      ci_type:     ci.ci_type,
  ...
  useEffect(() => {
    if (open) {
      reset({
        ...
        os_version:  ci.os_version ?? '',
        customer_id: ci.customer_id ?? '',
        description: typeof ci.attributes?.description === 'string'
          ? ci.attributes.description
          : '',
      });
    }
  ...
  async function onSubmit(values: EditCIValues) {
    try {
      const payload = {
        name:        values.name,
      ...
      await api.put(`/${tenantSlug}/cmdb/cis/${ci.id}`, payload);
      toast.success('CI가 수정되었습니다.');
      await queryClient.invalidateQueries({ queryKey: ['cmdb-ci-detail', tenantSlug, ci.id] });
