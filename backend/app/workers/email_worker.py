...
- tenant 조회: IMAP_TENANT_SLUG → DB에서 tenant_id 조회
- IMAP_HOST 미설정 시 조용히 sleep (crash 없음)
"""
...
async def _insert_ticket(
    ...
    async with AsyncSessionLocal() as session:
        row = await session.execute(
            ...
            {
                "tenant_id": tenant_id,
                "title": title[:500],          # 컬럼 제약 String(500)
...
async def _fetch_and_create_tickets() -> None:
    ...
    if not settings.IMAP_TENANT_SLUG:
        logger.warning("IMAP_TENANT_SLUG 미설정 — 수신 메일을 어느 테넌트에 생성할지 알 수 없음, 건너뜀")
        return

    # tenant_id 조회 (매 주기마다 재조회: 동적 등록 대응)
    ...
    # 동기 IMAP 작업을 executor에서 실행 (이벤트 루프 블로킹 방지)
    loop = asyncio.get_event_loop()
    try:
        mails = await loop.run_in_executor(None, _fetch_unseen_emails)
    except Exception:
        logger.exception("IMAP 연결/폴링 오류")
        return

    if not mails:
        logger.debug("UNSEEN 메일 없음")
        return

    logger.info("UNSEEN 메일 %d건 처리 시작", len(mails))
    for mail in mails:
        try:
...
async def main() -> None:
    ...
    if not settings.IMAP_HOST:
        logger.info("IMAP_HOST 미설정 — email_worker 비활성 (sleep 대기)")
        while _running:
            await asyncio.sleep(60)
        return

    logger.info(
        "Email worker starting — host=%s folder=%s interval=%ds",
        settings.IMAP_HOST,
